...
def get_mcore_model_from_nemo_file(nemo_restore_from_path, cpu_only=False):
    model_cfg = MegatronGPTRewardModel.restore_from(nemo_restore_from_path, return_config=True)
    ...
    mcore_model = MegatronGPTRewardModel(model_cfg, trainer=trainer)
    return mcore_model
...
def restore_model(nemo_file, cpu_only=False):
    ...
    model_config = MegatronGPTRewardModel.restore_from(
    ...
    if "target" in model_config and model_config.target.endswith("MegatronGPTSFTModel"):
        logging.warning(
            ...
            "This is a known issue. For now, please modify the config yaml file to use `MegatronGPTRewardModel`."
        )

    if model_config.precision in ['bf16', 'bf16-mixed']:
        model_config.megatron_amp_O2 = True

    model = MegatronGPTRewardModel.restore_from(
